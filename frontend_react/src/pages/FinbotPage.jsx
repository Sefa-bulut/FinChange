...
import { ask, getNews } from '../services/finbotService';
...
📰 Sentiment analizi: Son haberler` }
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (
        inputLower.includes('türk borsa haberleri') ||
        ...
        const data = await getNews(); // { success, news, total }
        if (data?.success && Array.isArray(data.news) && data.news.length > 0) {
          ...
          setMessages(prev => [...prev, { sender: 'bot', text: newsText, chart: null }]);
        } else {
          setMessages(prev => [...prev, { sender: 'bot', text: language === 'tr' ? '❌ Haber bulunamadı. Lütfen daha sonra tekrar deneyin.' : '❌ No news found. Please try again later.', chart: null }]);
        }
      } else {
        const data = await ask({ question: input, language }); 
        setMessages(prev => [...prev, { sender: 'bot', text: data?.answer || data?.error || 'Yanıt alınamadı.', chart: data?.chart || null }]);
      }
    } catch (error) {
      setMessages(prev => [...prev, { sender: 'bot', text: `Bir hata oluştu: ${error.message || 'Bilinmeyen hata'}`, chart: null }]);
    } finally {
      setLoading(false);
    }
  };
      ...
      return (
        <div>
          ...
          <ul style={{ paddingLeft: 18 }}>
            {newsList.map((n, i) => (
              <li key={i} style={{ marginBottom: 8 }}>{formatMessageWithLinks(n)}</li>
            ))}
          </ul>
        </div>
      );
    ...
    <div style={styles.container}>
      ...
      <div style={styles.chatArea}>
        {messages.map((msg, index) => (
          <div key={index} style={msg.sender === 'user' ? styles.userMessage : styles.botMessage}>
            ...
            {msg.chart && (
              <img src={`data:image/png;base64,${msg.chart}`} alt="Chart" style={styles.chartImage} />
            )}
          </div>
        ))}
        {loading && (
      ...
      <div style={styles.inputArea}>
        <input
          ...
          disabled={loading}
        />
        <button onClick={handleSend} style={styles.sendButton} disabled={loading || !input.trim()}>
          Gönder
        </button>
      </div>
    </div>
